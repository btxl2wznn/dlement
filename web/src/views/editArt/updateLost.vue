...
<template>
  <div class="editUser bg-gray-1 text-dark-1">
    <div class="user-header w-100 d-fx ai-center bg-white text-dark-1">
      ...
      <div class="f-xl ml-3 flex-1">编辑丢失</div>
      ...
      <div class="container w-100 mt-4">
        <van-form class="w-100" @submit="onSubmit">
          ...
          <van-field
            ...
            clickable
            name="area"
            :value="model.lostPlace"
            label="丢失地点"
            placeholder="点击选择"
            @click="showArea = true"
            :rules="[{ required: true, message: '不知道填不确定' }]"
          />
...
export default {
  ...
  methods: {
    ...
    getPlace(values) {
      this.model.lostPlace = values.pop();
      this.showArea = false;
    },
    ...
    async onSubmit(values) {
      this.model.lostTime = dayjs(this.model.foundTime).format("YYYY-MM-DD");
      this.model.luserId = JSON.parse(localStorage.getItem("userInfo")).id;
      const res = await this.$http.put(
        `rest/updateLost/${this.model.id}`,
        this.model
      );
      if (res.data.success) {
        this.$toast("修改成功");
    ...
    async getLost() {
      const found = await this.$http.get(`rest/webGetLost/${this.id}`);
