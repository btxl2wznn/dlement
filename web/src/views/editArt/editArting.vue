...
<template>
  <div class="articles">
    ...
    <div class="edit bg-blue-2 text-white mt-5 d-fx ai-center text-center">
      <div @click="determine()" class="edit-item">{{ kind === "lost" ? "继续挂失" : "继续招领" }}</div>
      <div class="edit-item">编辑</div>
    </div>
  </div>
</template>
        ...
        .then(async () => {
          if (this.kind == "lost") {
            const res = await this.$http.put(
              `rest/returnisFind/${this.article.id}`
            );
            if (res.data.success) {
              this.$toast("修改成功");
              this.$router.go(-1);
            ...
            const res = await this.$http.put(
              `rest/returnisGive/${this.article.id}`
            );
            if (res.data.success) {
              this.$toast("修改成功");
              this.$router.go(-1);
