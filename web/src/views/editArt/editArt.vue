...
<template>
  <div class="articles">
    ...
    <scroll class="deta-scroll" ref="scroll" :pull-upLoad="false" :scrollX="false">
      <div class="content pt-3 pl-2 pr-2">
        ...
        <div class="art f-xl mt-2">
          <div class="mt-2">
            <span class="text-dark-2">{{ kind === "lost" ? "丢失物品" : "拾取物品" }}</span>
            {{ article.gname }}
          </div>
          <div class="mt-2">
    ...
    <div class="edit bg-blue-2 text-white mt-5 d-fx ai-center text-center">
      <div @click="determine()" class="edit-item">{{ kind === "lost" ? "确认已找回" : "确认被领取" }}</div>
      <div @click="toUpdate()" class="edit-item">编辑</div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    async determine() {
      this.$dialog
        ...
        .then(async () => {
          if (this.kind == "lost") {
            const res = await this.$http.put(`rest/isFind/${this.article.id}`);
            if (res.data.success) {
              this.$toast("修改成功");
              this.$router.go(-1);
            }
          } else {
            const res = await this.$http.put(`rest/isGive/${this.article.id}`);
            if (res.data.success) {
              this.$toast("修改成功");
              this.$router.go(-1);
            }
    ...
    toUpdate() {
      if (this.kind == "lost") {
        this.$router.push(`/updateLost/${this.article.id}`);
      } else {
        this.$router.push(`/updateFound/${this.article.id}`);
      }
    },
    async fetch() {
      let res;
