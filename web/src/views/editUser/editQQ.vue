...
<template>
  <div class="editUser bg-gray-1 text-dark-1">
    <div class="user-header w-100 d-fx ai-center bg-white text-dark-1">
      ...
      <div class="f-xl ml-3 flex-1">修改QQ</div>
...
export default {
  ...
  methods: {
    ...
    async save() {
      var reg = /^[1-9][0-9]{4,10}$/; //验证规则 //验证手机号正则
      if (!reg.test(this.model.userQQ)) {
        this.$toast("请输入正确QQ号");
      } else {
        this.model.id = JSON.parse(localStorage.getItem("userInfo")).id;
        const res = await this.$http.put("/rest/updateQQ", this.model);
        if (res.data.success) {
